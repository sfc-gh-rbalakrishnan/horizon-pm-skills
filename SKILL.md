...
**Goal:** A living tracker of testable bets, each with explicit rejection criteria, biweekly customer meeting cadence, and a scored evidence gate that controls when a hypothesis can advance to Stage 3. Customer meetings are also where you collect raw JTBD material.

---

### 2A — Pre-Meeting Preparation
...
2. **For each hypothesis**, write:
   ...
   - **Rejection Criteria** — the most important part. Write before any interview:
     - *Validated if:* [a specific, observable customer behavior or statement]
     - *Rejected if:* [a specific, observable customer behavior or statement]
...
4. **Devil's advocate gate:** For each 🔴 Critical hypothesis, state the strongest reason it could be *wrong* before any interviews. Log this in the hypothesis doc. A hypothesis whose rejection criteria you cannot articulate is not ready for customer meetings.
...
**Validated if:** [observable customer behavior]
**Rejected if:** [observable customer behavior]
**Key Validation Questions:**
- [Question 1 — tests the core assumption]
- [Question 2 — tests severity/urgency]
- [Question 3 — tests scope/alternatives/build-vs-buy preference]
...
- **Part C (Platform & Architecture):** Probe technical requirements, integration constraints, policy granularity needs.
- **Part D (Commercial & Build vs. Buy):** Probe native vs. third-party preference, budget ownership, timeline to invest.

**Rule:** Never pitch a solution in Parts A or B. You are listening, not selling. Pitching before scoring contaminates the severity/urgency data.

---

### 2C — Post-Meeting Scoring and Hypothesis Update

After each meeting:

1. **Calculate Priority Score** for each problem scored: `Priority Score = Severity × Urgency` (max = 16)

| Tier | Score | Decision |
|------|-------|---------|
| Must-solve | > 12 | Target for Phase 1 |
| Should-solve | 8–12 | Phase 1 or Phase 2 based on complexity |
| Validate further | 4–8 | Defer; revisit after Phase 1 GA |
| Deprioritize | < 4 | Exclude from near-term roadmap |

2. **Update hypothesis status** in `hypotheses.md` — use average score across all customers interviewed so far:
   - **Confirmed** (avg priority score ≥ 8, consistent with Validated if criteria)
   - **Partially confirmed** (avg 4–7)
   - **Rejected** (avg < 4, or Rejected if criteria met)
   - **Insufficient data** (< 2 customers scored this hypothesis)

3. **Log to Calibration Log** in `hypotheses.md` — one row per meeting per hypothesis.

4. **Log raw JTBD candidates** in the Raw JTBD Harvest section — one row per trigger phrase heard.

---

### 2D — Evidence Gate: Required Before Stage 3 (Heilmeier)

This gate is binary. All items must be checked before writing the Heilmeier. If any item is unchecked, continue the biweekly interview cycle. Do not rationalize partial evidence into a "close enough."

**Run this checklist at the end of every biweekly cycle:**

```
EVIDENCE GATE — Required before Stage 3

[ ] ≥10 customer interviews completed total (or ≥2 for a single targeted hypothesis)
[ ] All 🔴 Critical hypotheses have status Confirmed, Partially confirmed, or Rejected — none remain "Insufficient data"
[ ] Average Priority Score calculated across all customers for all problems
[ ] At least 3 problems score in Must-solve tier (> 12)
[ ] No Must-solve problem has a known technical impossibility for the planned phase
[ ] Problem-to-capability mapping complete for all Must-solve problems
[ ] At least 2 customers have committed as design partners (committed = agreed to a specific deliverable, not just "interested")
[ ] Key target verticals represented in the interview set (e.g., FSI + Healthcare if that is the target)
[ ] Top 3 problems are consistent across at least 2 different verticals
[ ] At least 5 customers confirmed preference for native/Snowflake solution over third-party alternatives
[ ] Thesis Validation Tracker updated: every thesis is Confirmed / Partially confirmed / Rejected (no blanks)
```

**Gate passes →** Proceed to Stage 3: Heilmeier Product Review.
**Gate fails →** Continue biweekly interviews. Do not write the Heilmeier. Note specifically which items are blocking and design the next interview set to address them.

**⚠️ STOPPING POINT**: Present gate checklist status to user. If the gate passes, explicitly confirm before proceeding. If any 🔴 Critical hypothesis is still "Insufficient data," require the PM to either (a) run more interviews targeting that hypothesis or (b) explicitly retire it with a rationale. A Critical hypothesis you have not tested is a risk you are carrying silently into engineering.

---

**Include in hypotheses.md:** Calibration Log table + Decision Rules (when to build vs. retire a hypothesis) + Raw JTBD Harvest table + Problem Priority Score summary table (updated after each interview round)
